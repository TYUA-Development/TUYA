...
// FixedMoveObject와 동일한 정착(settle) 동작을 하지만, 트리거 조건이 "특정 Collider와 충돌"이
// 아니라 "등록된 Rope가 끊어짐(Rope.IsCut)"이다. 물리로 자연스럽게 낙하하다가 등록된 Rope가
// 끊어지는 순간 Kinematic으로 전환하고, 오브젝트 자신의 원점(transform.position)이
// targetPosition으로, 각도가 targetAngle로 향하도록 서서히 맞춘다. 회전은 pivot(축)을
// 중심으로 일어나 자연스럽게 보이지만, 최종적으로 targetPosition/targetAngle에 도달하는 것은
// pivot이 아니라 오브젝트 원점이다. 이를 위해 "오브젝트가 targetPosition/targetAngle에
// 도달했을 때 pivot이 있어야 할 위치"를 먼저 역산해두고, pivot이 그 지점으로 이동하도록
// 보간한다.
[RequireComponent(typeof(Rigidbody2D))]
public class FixedMoveObject_Rope : MonoBehaviour
{
    [Header("Trigger")]
    [Tooltip("감시할 Rope. 이 Rope가 끊어지는(IsCut) 순간 물리 낙하를 멈추고 Kinematic으로 전환한 뒤 아래 목표 위치/각도로 서서히 자세를 맞춥니다.")]
    public Rope rope;
    ...
    [Tooltip("Rope가 끊어진 뒤 오브젝트(Transform.position)가 최종적으로 위치해야 할 월드 좌표")]
    public Vector2 targetPosition;

    [Tooltip("Rope가 끊어진 뒤 오브젝트의 최종 각도 (Z축, degree)")]
    public float targetAngle;

    [Tooltip("목표 위치/각도로 맞춰지는 데 걸리는 시간(초)")]
